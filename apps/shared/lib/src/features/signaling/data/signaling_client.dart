...
class SignalingClient {
  ...
  Future<RTCSessionDescription?> getOffer(
    String deviceId,
    String callId,
  ) async {
  ...
  Stream<RTCSessionDescription?> watchOffer(String deviceId, String callId) =>
      _call(
        deviceId,
        callId,
      ).snapshots().map((doc) => _toDescription(doc.data()?['offer']));
  ...
  Stream<RTCSessionDescription?> watchAnswer(String deviceId, String callId) =>
      _call(
        deviceId,
        callId,
      ).snapshots().map((doc) => _toDescription(doc.data()?['answer']));
